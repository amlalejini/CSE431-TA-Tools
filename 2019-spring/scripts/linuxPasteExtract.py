...
def main():
    ...
    for student in students:
        ...
        for problem in problems:
            ...
            problem_name = '.'.join(problem.split(".")[:-1])
            ...
            for line in pasty:
                line = line.strip("\n")
                line = line.strip("\r")
                if "Join us on IRC at #hackerrank" in line or "Contest Calendar | Interview Prep" in line:
                    break
                elif fcode:
                    if len(line) > 0:
                        if line.isalnum(): continue
                        else:
                            #encoding broke stuff for some reason?
